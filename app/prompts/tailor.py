...
TAILOR_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "Bolding: inside each bullet, wrap every concrete, specific, named technology "
            "— a programming language, framework, library, tool, platform, or database "
            "(e.g. 'React', 'Kafka', 'PostgreSQL', 'AWS', 'Docker') — in HTML "
            "<strong>...</strong> tags. Use ONLY the <strong> HTML tag, never Markdown "
            "'**' (Markdown will render as literal asterisks). Do NOT bold vague umbrella "
            "phrases, methodologies, concepts, or buzzwords — the same reject list as rule "
            "#4 applies (e.g. 'distributed systems', 'scalable', 'microservices "
            "architecture', 'system design'). This bolding is purely visual formatting: it "
            "does NOT add, change, or invent any content, so it never violates the "
            "no-fabrication rule. Stay restrained — bold only genuine technology names, not "
            "whole phrases, so a bullet never looks like a wall of bold. When in doubt "
            "about whether a term is a specific named technology, leave it unbolded.\n\n"
            "4. skills — Additive but CONCISE. First, keep every single skill already "
            "listed in the resume — none may be dropped. Then you may append a FEW missing "
            "skills, but ONLY when a keyword is a concrete, specific, named technology — a "
            "programming language, framework, library, tool, platform, or database (e.g. "
            "'Kafka', 'PostgreSQL', 'React'). Do NOT append vague umbrella phrases, "
            "methodologies, or buzzwords even if they appear in the keyword list — e.g. "
            "reject things like 'distributed systems', 'scalable systems', 'system design', "
            "'microservices architecture', 'object-oriented design', 'software development "
            "lifecycle', 'SDLC', 'on-call operations', 'database systems', 'version control', "
            "'debugging', 'AI-powered tools'. Never append a near-duplicate of a skill already "
            "listed. Keep the skills section tight: it should read like a real resume's skills "
            "block, not an exhaustive keyword dump. Group all skills into sensible categories, "
            "and prefer a small number of focused categories.\n\n"
